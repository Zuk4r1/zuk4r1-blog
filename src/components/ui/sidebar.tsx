...
export function SidebarOverlay({ onClose }: { onClose: () => void }) {
  return (
    ...
    >
      ...
      />
      <motion.aside 
        className="absolute left-0 top-0 bottom-0 w-72 glass-panel border-r border-cyber-border flex flex-col overflow-hidden shadow-2xl"
        initial={{ x: '-100%' }}
        animate={{ x: 0 }}
        exit={{ x: '-100%' }}
        transition={{ type: 'spring', stiffness: 300, damping: 30 }}
      >
        {/* Efecto de entrada de hangar */}
        <motion.div 
          className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-cyber-primary to-transparent"
          initial={{ opacity: 0 }}
        ...
        <div className="flex items-center justify-between p-4 border-b border-cyber-border/30 bg-cyber-primary/5">
          ...
          <motion.button 
            onClick={onClose} 
            className="text-cyber-muted hover:text-cyber-primary transition-colors p-2 hover:bg-cyber-primary/10 rounded-md"
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-8 text-center border-b border-cyber-border/30 relative">
            ...
            <motion.img 
              src={perfil} 
              alt="Perfil" 
              className="w-24 h-24 mx-auto rounded-full border-2 border-cyber-primary mb-4 avatar-glow" 
