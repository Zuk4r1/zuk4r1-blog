...
import { getPublishedPosts, getPostsByTag, Post as PostType } from '@/lib/posts';
...
export function Tags() {
  ...
  usePostsSubscription((updated) => {
    computeCounts(updated.filter((p) => p.published));
  });
  ...
  return (
    <motion.div
      ...
      className="container mx-auto px-4 py-8 readable-list"
    >
      {/* Header */}
      <div className="mb-12">
        <h1 className="text-4xl font-bold text-white glow-text mb-4">Etiquetas</h1>
          ...
          {tagCounts.map((tag) => (
            ...
            >
              <button
                ...
                className="chip-3d chip-3d-lg font-mono"
              >
                {tag.name} ({tag.count})
              </button>
            </motion.div>
