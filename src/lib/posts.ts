...
function getMarkdownModules(): Record<string, string> {
  try {
    ...
    logger.error('posts.getMarkdownModules', 'Error al obtener módulos markdown', error);
    return {};
  }
}
...
function parseMarkdownFileSync(filePath: string, fileContent: string): Post | null {
  try {
    ...
    logger.error('posts.parsePost', `Error al parsear archivo ${filePath}`, error);
    return null;
  }
}
...
function loadPostsFromMarkdownSync(): Post[] {
  try {
    ...
    logger.error('posts.loadPostsSync', 'Error al cargar posts desde markdown', error);
    return [];
  }
}
...
export async function notifyPostsUpdate(): Promise<void> {
  ...
  for (const cb of subscribers) {
    try {
      ...
      logger.error('posts.notifyPostsUpdate', 'Error notificando suscriptor', e);
    }
  }
}
